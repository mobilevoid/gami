...
def compute_graph_fingerprint(
    ...
    for neighbor_id in one_hop_neighbors:
        neighbor_edges = [e for e in edges if e["source"] == neighbor_id or e["target"] == neighbor_id]
...
def fingerprint_similarity(fp1: GraphFingerprint, fp2: GraphFingerprint) -> float:
    ...
    # Weight edge types more heavily
    similarity = 0.3 * out_edge_sim + 0.3 * in_edge_sim + 0.4 * type_sim
...
def compute_relation_score(
    ...
    struct_sim = fingerprint_similarity(query_fingerprint, candidate_fingerprint)
    ...
    # Combined score
    score = 0.4 * struct_sim + 0.3 * neighbor_score + 0.3 * path_score
